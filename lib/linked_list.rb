class LinkedList
  ...
  def insert(position, text)
    ...
    if position + 1 > self.count
      ...
      first = node_position(@head, position - 1)
  ...
  def counter(node, count)
    if node.last_node? == true
      ...
      counter(node.next_node, count += 1)
    end
  end
  ...
  def to_string
    if @head == nil
      ...
      string(@head.next_node, "#{head.data}")
    end
  end
  ...
  def find(position, strings)
    ...
    if strings == 1
      ...
      string(node.next_node, "#{node.data}", strings -= 1)
    end
  end
  ...
  def data_finder(node, data)
    if node.data == data
      ...
      data_finder(node.next_node, data)
    end
  end
